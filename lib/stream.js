...
module.exports = function(root, opts) {
  ...
  return function(req, res, next) {
    res.stream = function() {
      var arg, args, cb, opt, src, _i, _len;
...
Stream = (function() {
  ...
  Stream.prototype.parseRange = function(stat, req) {
    var end, ini, range, ranges, _i, _len, _ref, _ref1, _ref2;
  ...
  Stream.prototype.serve = function(src, opt, cb, req, res, next) {
    ...
    return this.store.fd.get(src, function(err, fd) {
      ...
      return _this.store.stat.get("" + fd + ":" + src, function(err, stat) {
        var buf, end, etag, gzbuf, gzstream, ini, isFirstStream, match, partial, range, since, storekey, stream, _ref, _ref1, _ref2, _ref3;
        if (err) {
          cb(err, null);
          return _this.error(err, res, next, fdend);
        }
        range = _this.parseRange(stat, req);
        if (range === null) {
          partial = false;
        ...
        res.setHeader('cache-control', ((_ref2 = opt.headers) != null ? _ref2['cache-control'] : void 0) || 'public');
        res.setHeader('last-modified', stat.mtime.toUTCString());
        res.setHeader('etag', etag);
        res.setHeader('content-type', ((_ref3 = opt.headers) != null ? _ref3['content-type'] : void 0) || mime.lookup(path.extname(src)));
        if (!partial) {
          res.statusCode = 200;
        } else {
          res.statusCode = 206;
