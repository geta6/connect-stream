...
module.exports = function(root, opts) {
  ...
  return function(req, res, next) {
    res.stream = function(src) {
      return stream.serve(src, req, res, next);
    };
    return next();
  };
};
...
Stream = (function() {
  ...
  Stream.prototype.cache = function(fdend, res) {
    fdend();
    res.statusCode = 304;
    return res.end();
  };

  Stream.prototype.serve = function(src, req, res, next) {
    var _this = this;
    if (!src) {
      throw Error('no src');
    }
    ...
    return this.store.fd.get(src, function(err, fd) {
      ...
      return _this.store.stat.get("" + fd + ":" + src, function(err, stat) {
        var buf, end, etag, gzbuf, gzstream, ini, match, partial, range, since, storekey, stream, _ref, _ref1;
        if (err) {
